...
void main() {
  ...
  setUp(() {
    ...
    useCase = SearchPicturesByTitle(mockPictureOfTheDayRepository);
  });
  ...
  String mockedTitle = 'title';
  Params mockedParams = Params(title: mockedTitle);
  PictureFailure mockedFailure = ServerUnavaiableFailure();

  group('searchPicturesByTitle useCase', () {
    test(
      "should call searchPicturesByTitle from pictureOfTheDayRepo",
      () async {
        // arrange
        when(mockPictureOfTheDayRepository.searchPictureByTitle(mockedTitle)).thenAnswer((_) async => Right(pictures));
        // act
        await useCase(mockedParams);
        // assert
        verify(mockPictureOfTheDayRepository.searchPictureByTitle(mockedTitle)).called(1);
      },
    );
    test(
    ...
    test(
      ...
      () async {
        // arrange
        when(mockPictureOfTheDayRepository.searchPictureByTitle(mockedTitle)).thenAnswer((_) async => Left(mockedFailure));
        // act
        final result = await useCase(mockedParams);
        // assert
